...
## Overview
`MemUnit` is a manager class responsible for creating, storing, and deleting memory contexts. It supports multiple context types, including `PagedContext` and `SequentialPageContext`, and provides lookup and lifecycle management for these contexts. `MemUnit` is the main entry point for higher-level memory management in the experiment.
...
- `SequentialPageContext`: Sequential page-based context (from `context.h`)
...
### CreateSequentialPageContext
```
void CreateSequentialPageContext(word_t id);
```
- **Description:** Creates a new `SequentialPageContext` with the given ID. Throws `std::invalid_argument` if a context with the same ID already exists.
...
unit.CreateSequentialPageContext(77);
auto ctx1 = unit.GetContext(42);
auto ctx2 = unit.GetContext(77);
unit.DeleteContext(42);
unit.DeleteContext(77);
```
...
- [Accessors: MemSlice & SequentialPageMemAccess](memaccess.md)
...
- Integration with PagedContext, SequentialPageContext, and accessors
